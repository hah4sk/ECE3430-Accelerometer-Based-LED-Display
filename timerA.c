...
if(info->LEDState != Uncalibrated){
	...
	for(i=0; i < 8; i++){
		if (info->Onticks[i] > 0) {
			info->Onticks[i]--; //decrease amount of ticks remaining

			if(info->Onticks[i] == 0){
				//if done, set its corresponding spot in LedStatus to 0

				info->LedStatus &= ~(0x01<<i);
...
//				}
			...
			if(info->LedStatus == 0 && info->ticksDone == info->Period){ //if everything is off and period limit is reached, refresh state
				mainLED = -1;
			}
